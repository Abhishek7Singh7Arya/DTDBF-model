...
void runExperiment(std::vector<T>& keys,
    ...
    // proteus::Proteus proteusInstance; // Create a Proteus instance
    // uint64_t t = proteusInstance.processKey<T>(keys[0], 6); // Call processKey using the instance
    // std::cout << "original trie " << keys[0] << " edited trie " << t << std::endl;
    
     std::tuple<size_t, size_t, size_t> parameters = proteus::modeling(keys, sample_queries, bpk, keylen);
        end = clock();
        unsigned long modeling_time = 1000*(end-begin)/CLOCKS_PER_SEC;
        SAVE_TO_RESCSV(modeling_time)
        ...
        SAVE_TO_RESCSV(std::get<0>(parameters))
        SAVE_TO_RESCSV(std::get<2>(parameters))
...
int main(int argc, char **argv) {
    ...
    rescsv.open(argv[5],std::ios_base::app);
