...
experiment() {
    ...
    if [ $filter = "SuRF" ]; then
        ...
        $EXP_BIN "$IS_INT_BENCH" "$filter" "$surfhlen" "$surfrlen" "$res_csv" >> ./experiment_result
        ...
        $EXP_BIN "$IS_INT_BENCH" "$filter" "$membudg" "$sampleprop" "$res_csv" >> ./experiment_result
    fi

    echo -e "### END EXPERIMENT ###" >> ./experiment_result
    cp ./experiment_result "$res_folder/logdata.txt"
