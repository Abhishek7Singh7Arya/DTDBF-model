...
//             size_t n = seeds32_.size();

//             uint64_t edited_key = 0ULL;
//             // size_t slid_window_size = 64-prefix_len_+1;
...
//                 if(qtt)
//                 {   
//                     cout <<"mask prefix "<<mask_prefix<<endl;
//                     std::cout<<" key "<<key << " size of seeds "<<n<<"mask prefix "<<mask_prefix << " reverse" << keyreverse << std::endl; 
//                 }
                
//                 edited_key = keyreverse;
      
//                 // if(qtt)  cout<<"debug " <<(reverse_64(edited_key & mask_prefix)>>(64-prefix_len_))<<endl;
//                 // out &=get(hash( reverse_64(edited_key& mask_prefix)>>(64-prefix_len_), seeds32_[0] ) );
//                 // edited_key>>=number_of_bits;

//                 // if(qtt)  cout<<"debug " <<(reverse_64(edited_key& mask_prefix)>>(64-prefix_len_))<<endl;
//                 // out &=get(hash( reverse_64(edited_key& mask_prefix)>>(64-prefix_len_), seeds32_[1] ) );
//                 // edited_key>>=number_of_bits;

//                 //      if(qtt)  cout<<"debug " <<(reverse_64(edited_key& mask_prefix)>>(64-prefix_len_))<<endl;
//                 // out &=get(hash( reverse_64(edited_key& mask_prefix)>>(64-prefix_len_), seeds32_[2] ) );
//                   edited_key>>=1;
//                   uint64_t temp ;  
//                   if(prefix_len_+1<=64)
...
//                         if(qtt) cout<<"debug key query" << temp <<endl;
//                      }
                    
               
//                 for(size_t i=n/2 ; i< n && out ; i++)
//                 {
//                     out &=get(hash(key>>(64-prefix_len_),seeds32_[i]));
//                      if(qtt)
//                     std::cout<<" orignal "<<key<<" key "<<( key>>(64- prefix_len_))<<" i "<<i<<std::endl;
...
bool PrefixBF::Query(const uint64_t key, bool shift) 
{
    ...
    uint64_t k =   shift ? key >> (64-temp):key;
    size_t n = seeds32_.size();
            ...
            for(size_t i = 0 ; i< prefix_len_ ; i++)
            {
                ...
                for(size_t i =0 ; i<(n/2) && out ;i++)
                {
                    out &=get(hash(k&Local_mask,seeds32_[i]));
                    if(qtt) std::cout<<" orignal "<<key<<" key "<<k<<" edited "<< (k&Local_mask)<<" i "<<i<<" mask "<<Local_mask<<std::endl;
                }
...
bool PrefixBF::Query(const uint64_t from, const uint64_t to) {
    ...
    uint64_t iterator = from >>(64-temp);
    // iterator&=Local_mask;
    uint64_t upper_bound = (to - 1) >> (64-temp) ;
    // upper_bound&=Local_mask;

      if(qtt) std::cout<<"query orignal "<<from<<" to "<<to<<" edited "<< iterator<<" to "<<upper_bound<<" prefix_len_ "<<prefix_len_<<std::endl;
    while (iterator <= upper_bound) {
