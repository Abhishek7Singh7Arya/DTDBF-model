...
std::pair<int, int> KQlcp(const std::vector<uint64_t>& keys,
    ...
    */
    ...
    if (qleft >= *kleft && kright != keys.cend() && qright <= *kright) {
        ...
        return std::make_pair(longestCommonPrefix(    processKey  (qleft),  processKey   (*kleft), max_klen), 
                              longestCommonPrefix(   processKey  (qright - 1),    processKey (*kright), max_klen));
    } else if (qleft < *kleft && qright <= *kleft) {
        // QLeft QRight KMin
        return std::make_pair(-1, longestCommonPrefix(  processKey   (qright - 1),  processKey   (*kleft), max_klen));
    } else if (kright == keys.cend() && keys.back() < qleft) {
        // KMax QLeft QRight
        return std::make_pair(longestCommonPrefix(  processKey   (qleft),  processKey   (keys.back()), max_klen), -1);
    } else {
        assert(false);
    }
...
std::tuple<size_t, size_t, size_t, long double, size_t, size_t> find_best_conf(const std::vector<size_t>& trconfs,
    ...
    for (auto const & q : sample_queries) {
        //  T q_first = processKey(q.first);
        // T q_second = processKey(q.second);
        ...
        int Qlcp = longestCommonPrefix(  processKey (q.first),  processKey  (q.second), max_klen);
    ...
    }  proteus::setMask(best_trie_depth);
printf(" arya best config %zu %zu \n",best_trie_depth,best_bflen);
...
std::tuple<size_t, size_t, size_t> modeling(const std::vector<T>& keys, 
    ...
    for(auto k : keys)
    {
        keyy.push_back(processKey(k));
    }
    std::sort(keyy.begin(),keyy.end());

    START_MODEL_TIMER
    countUniqueKeyPrefixes(key_prefixes, keyy, max_klen);
